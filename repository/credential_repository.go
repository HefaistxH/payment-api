...
func (r *credentialRepository) GetCredByEmail(email string) (entity.Credential, error) {
	...
	err := r.db.QueryRow(config.GetCredByEmailQuery, email).Scan(&credential.Id, &credential.UserId, &credential.Email, &credential.Password, &credential.Pin, &credential.Role)
